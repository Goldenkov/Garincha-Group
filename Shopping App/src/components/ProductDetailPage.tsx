...
export default function ProductDetailPage({
  ...
  return (
    <div className="bg-[#ffffff] relative size-full">
      ...
      {/* Product Details */}
      <div className="absolute left-0 right-0 top-[449px] bottom-[120px] overflow-y-auto">
        <div className="px-6 py-6 space-y-6">
          {/* Product Info */}
          <div>
            <p className="text-[18px] font-['Inter:Regular',_sans-serif] text-[#8B4513] mb-2 font-medium">
              {product.price}
            </p>
            ...
            {/* Bambicoins Reward */}
            <div className="flex items-center gap-2 mt-2 bg-[#FFD700] rounded-lg px-3 py-2 inline-flex">
              <span className="text-[16px]">🪙</span>
              <span className="text-[14px] text-[#8B4513] font-medium">
                Получите +{product.bambicoins * quantity} бамбикоинов
              </span>
            </div>
          </div>
          ...
          {/* Description */}
          <div>
            <h3 className="text-[16px] font-['Inter:Medium',_sans-serif] text-[#000000] mb-2">Описание</h3>
            <p className="text-[16px] font-['Inter:Regular',_sans-serif] text-[#000000] leading-[24px] mb-4">
              {product.description}
            </p>
            <p className="text-[14px] font-['Inter:Regular',_sans-serif] text-[#757575] leading-[20px]">
              {product.location}
            </p>
          </div>
          ...
          {product.specifications.length > 0 && (
            <div>
              <h3 className="text-[16px] font-['Inter:Medium',_sans-serif] text-[#000000] mb-2">Характеристики</h3>
              <ul className="space-y-1">
                {product.specifications.map((spec, index) => (
                  <li key={index} className="text-[14px] font-['Inter:Regular',_sans-serif] text-[#757575] flex items-center">
                    <span className="w-2 h-2 bg-[#8B4513] rounded-full mr-2 flex-shrink-0"></span>
                    {spec}
                  </li>
                ))}
              </ul>
      ...
      {/* Footer */}
      <div className="bg-[#ffffff] shadow-[0px_0px_20px_0px_rgba(0,0,0,0.1)] absolute bottom-0 left-0 right-0 h-[120px]">
          ...
          <div className="box-border content-stretch flex flex-col gap-4 items-start justify-start p-0 relative w-full">
            ...
            <div className="flex items-center justify-between w-full">
              <div className="flex items-center gap-4">
                <span className="text-[14px] text-gray-600">Количество:</span>
              ...
              <div className="text-right">
                <p className="text-[18px] font-['Inter:Medium',_sans-serif] text-[#8B4513]">
                  {new Intl.NumberFormat('ru-RU', { 
                    style: 'currency', 
                    currency: 'RUB',
                    minimumFractionDigits: 0
                  }).format(product.priceValue * quantity)}
                </p>
              </div>
            </div>
            
            {/* Add to Basket Button */}
            <div className="basis-0 bg-[#8B4513] grow min-h-px min-w-px relative rounded-lg shrink-0 w-full">
              <button
                onClick={handleAddToCart}
                className="flex flex-row items-center relative size-full hover:bg-[#A0522D] transition-colors"
              >
                <div className="box-border content-stretch flex flex-row gap-2 items-center justify-start pl-6 pr-4 py-3 relative w-full">
                  <div className="basis-0 flex flex-col font-['Inter:Regular',_sans-serif] font-normal grow justify-center leading-[0] min-h-px min-w-px not-italic relative shrink-0 text-[#ffffff] text-[16px] text-left tracking-[-0.16px]">
                    <p className="block leading-[24px] text-[16px]">Добавить в корзину</p>
                  </div>
...
function Header({ cartCount, totalBambicoins, onBack, onMenuClick, onCartClick, productName }: HeaderProps) {
  return (
    <div className="absolute bg-[#ffffff] h-[149px] left-0 right-0 top-0">
      ...
      <div className="absolute bg-[#ffffff] h-16 left-0 overflow-clip right-0 top-[31px]">
        ...
        <div
          className="absolute flex flex-col font-['Newsreader:Medium',_sans-serif] font-medium justify-center leading-[0] text-[#8B4513] text-[24px] text-center text-nowrap top-9 tracking-[-0.24px] translate-x-[-50%] translate-y-[-50%]"
          style={{ left: "calc(50% - 0.5px)" }}
        >
          <p className="adjustLetterSpacing block leading-none whitespace-pre text-[20px]">Домашний Уют</p>
        </div>
        ...
        <div className="box-border content-stretch flex flex-col gap-4 items-end justify-end p-0 relative w-full">
          <div className="relative shrink-0 w-full">
            <div className="box-border content-stretch flex flex-row font-normal items-center justify-start leading-[0] p-0 relative text-left w-full">
              <div className="basis-0 flex flex-col font-['Newsreader:Regular',_sans-serif] grow justify-center min-h-px min-w-px relative shrink-0 text-[#000000] text-[20px] tracking-[-0.40px]">
                <h1 className="block leading-[28px] text-[20px] pr-4">{productName}</h1>
              </div>
              <button 
                onClick={onBack}
                className="cursor-pointer flex flex-row items-center gap-1 font-['Inter:Regular',_sans-serif] justify-center not-italic relative shrink-0 text-[#8B4513] text-[14px] text-nowrap tracking-[-0.14px]"
              >
                <svg width="12" height="12" viewBox="0 0 12 12" fill="none" className="shrink-0">
                  <path d="M7.5 2.5L4 6L7.5 9.5" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
                </svg>
                <span className="adjustLetterSpacing leading-[24px]">Назад</span>
              </button>
            </div>
          </div>
        </div>
